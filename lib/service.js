...
function Service (opts) {
  ...
  this.allowedLoopbackDevices = opts.allowedLoopbackDevices || []
  this.listenOnlyOnHost = opts.listenOnlyOnHost || false
...
Service.prototype._records = function () {
  ...
  Object.keys(interfaces).forEach(function (name) {
    interfaces[name].forEach(function (addr) {
      if (addr.internal && !self.allowedLoopbackDevices.includes(addr.address)) return
      if (self.listenOnlyOnHost && !addr.address == self.host) return
      if (addr.family === 'IPv4') {
